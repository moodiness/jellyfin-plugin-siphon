...
public sealed class AddonRegistry(StremioClient client, ConfigurationAccessor configuration, ILogger<AddonRegistry> logger)
{
    ...
    public async Task<IReadOnlyList<RegisteredAddon>> GetEnabledAsync(CancellationToken ct)
    {
        var addons = configuration.Current.Addons.Where(a => a.Enabled).Take(256).ToArray();
        ...
        {
            var addon = addons[i];
            var key = addon.Id + ":" + Digest(addon.ManifestUrl);
            try
            {
